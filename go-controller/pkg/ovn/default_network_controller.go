...
func (h *defaultNetworkControllerEventHandler) syncRemoteTrustZones(localNodes []*corev1.Node, remoteNodes []*corev1.Node) []error {
	...
	for _, localNode := range localNodes {
		for _, remoteNode := range remoteNodes {
			inSharedTransportZone, err := util.NodesInSharedTransportZone(localNode, remoteNode)
			if err != nil {
				aggregatedErrors = append(aggregatedErrors, fmt.Errorf("failed to check if node %s is in shared transport zone: %w", remoteNode.Name, err))
			} else {
				if inSharedTransportZone {
	...
	case factory.NodeType:
		...
		if newNodeIsLocalZoneNode {
			...
			syncZoneIC = syncZoneIC || h.oc.isLocalZoneNode(oldNode) || nodeSubnetChange || zoneClusterChanged ||
				switchToOvnNode || nodeEncapIPsChanged || nodePrimaryDPUHostAddrChanged || nodeTransportZonesChanged
			if syncZoneIC {
